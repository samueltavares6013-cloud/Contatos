...
const OPERATIONAL_PHONE_MIGRATION_KEY = 'centro_inteligencia_operacional_sem_numero_v1';
const EXTRA_EMERGENCY_TOPICS_MIGRATION_KEY = 'centro_inteligencia_subprefeituras_outros_v1';
const EXTERNAL_CONTACTS_MIGRATION_KEY = 'centro_inteligencia_contatos_externos_planilha_v1';
const defaultAdministrators = [{id:1,name:'Administrador principal',username:'admin',password:'admin123'}];
const categoryOrder = ['Emergência', 'Operacional', 'Outros'];
const categoryLabels = {'Emergência':'EXTERNOS','Operacional':'INTERNOS'};
const seedContacts = [
  ...
  {id:8,name:'Segurança',role:'Central de Segurança',phone:'',category:'Operacional',icon:'security',order:1,subcontacts:[{id:1,name:'Segurança',role:'Central de Segurança',phone:'(11) 99999-0010'}]},
  {id:9,name:'TI / Suporte',role:'Tecnologia da Informação',phone:'',category:'Operacional',icon:'computer',order:2,subcontacts:[{id:1,name:'TI / Suporte',role:'Tecnologia da Informação',phone:'(11) 99999-0011'}]},
  {id:10,name:'Manutenção',role:'Central de Manutenção',phone:'',category:'Operacional',icon:'tools',order:3,subcontacts:[{id:1,name:'Manutenção',role:'Central de Manutenção',phone:'(11) 99999-0012'}]},
  ...
  {id:103,name:'SUBPREFEITURAS',role:'Atendimento regional',phone:'',category:'Emergência',icon:'business',order:7,subcontacts:[]},
  {id:104,name:'OUTROS',role:'Contatos diversos',phone:'',category:'Emergência',icon:'person',order:8,subcontacts:[]}
];
const externalTopicContacts = {
  'Polícia Militar': [
    {name:'TEN CORONEL CAPARROZ',role:'SECRETARIA DE SEGURANÇA',phone:'(11) 98442-7760'},
    {name:'CAPITÃO CRÚVEL',role:'CPA/M-1',phone:'(11) 98331-8317'}
  ],
  'GCM': [
    {name:'INSPETOR GONÇALVES',role:'INTELIGÊNCIA GCM',phone:'(11) 99092-0322'}
  ],
  'SUBPREFEITURAS': [
    {name:'DAYANE GODOI',role:'SUB PREFEITURA BUTANTÃ',phone:'(11) 98672-9288'}
  ],
  'OUTROS': [
    {name:'MÁRIO CARDOSO',role:'TIC TRENS',phone:'(11) 99613-8990'},
    {name:'EPOMINONDAS',role:'SEC TRANSPORTES METROPOLITANOS',phone:'(11) 99874-0990'},
    {name:'LARISSA',role:'METRÔ SP',phone:'(11) 97693-6121'},
    {name:'FERREIRA',role:'TRIVIA TRENS',phone:'(11) 98234-9124'},
    {name:'RODRIGO',role:'METRÔ SP',phone:'(11) 98750-5091'},
    {name:'SERGIO',role:'INTELIGÊNCIA CPTM',phone:'(11) 96756-7971'},
    {name:'EVERTON DANTAS',role:'CCO PALMEIRAS',phone:'(11) 95889-0627'},
    {name:'GILBERTO',role:'SEGURANÇA PALMEIRAS',phone:'(11) 94073-2019'},
    {name:'WAGNER',role:'SPTURIS',phone:'(11) 97210-5785'},
    {name:'ALEXSANDER PARPINELLI',role:'ENGENHARIA CET',phone:'(11) 99776-5850'},
    {name:'SERAFIM',role:'ARTESP',phone:'(11) 99224-5122'},
    {name:'LUCAS DANTAS',role:'SECRETARIA DE CULTURA SP',phone:'(11) 94488-4814'},
    {name:'CORONEL PRATES',role:'SPTURIS',phone:'(11) 96604-5907'}
  ]
};
...
const openCategories = new Set(['Emergência']);
const openContacts = new Set();
const $ = (selector) => document.querySelector(selector);

function loadContacts(){
  try { const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)); return normalizeContacts(Array.isArray(saved)?saved:seedContacts); }
  catch { return normalizeContacts(seedContacts); }
}
function normalizeContacts(list){
  const normalized=JSON.parse(JSON.stringify(list)).filter(contact=>contact.category!=='Diretoria').map(contact=>({
    ...contact,
    subcontacts:Array.isArray(contact.subcontacts)?contact.subcontacts.map(item=>({...item,ramal:item.ramal||'',sso:item.sso||''})):[{id:1,name:contact.name,role:contact.role||'',phone:contact.phone,ramal:'',sso:''}]
  }));
  try {
    ...
    if(!localStorage.getItem(OPERATIONAL_PHONE_MIGRATION_KEY)){
      normalized.filter(contact=>contact.category==='Operacional').forEach(contact=>{ contact.phone=''; });
      localStorage.setItem(OPERATIONAL_PHONE_MIGRATION_KEY,'1'); localStorage.setItem(STORAGE_KEY,JSON.stringify(normalized));
    }
    if(!localStorage.getItem(EXTRA_EMERGENCY_TOPICS_MIGRATION_KEY)){
      seedContacts.filter(topic=>['SUBPREFEITURAS','OUTROS'].includes(topic.name)).forEach(topic=>{
        if(!normalized.some(contact=>contact.name.toLocaleUpperCase('pt-BR')===topic.name))normalized.push(JSON.parse(JSON.stringify(topic)));
      });
      localStorage.setItem(EXTRA_EMERGENCY_TOPICS_MIGRATION_KEY,'1'); localStorage.setItem(STORAGE_KEY,JSON.stringify(normalized));
    }
    if(!localStorage.getItem(EXTERNAL_CONTACTS_MIGRATION_KEY)){
      Object.entries(externalTopicContacts).forEach(([topicName,items])=>addSubcontactsToTopic(normalized,topicName,items));
      localStorage.setItem(EXTERNAL_CONTACTS_MIGRATION_KEY,'1'); localStorage.setItem(STORAGE_KEY,JSON.stringify(normalized));
    }
  }catch{}
  return normalized;
}
function addSubcontactsToTopic(list,topicName,items){
  const topic=list.find(contact=>contact.name.toLocaleUpperCase('pt-BR')===topicName.toLocaleUpperCase('pt-BR'));
  if(!topic)return;
  topic.subcontacts=Array.isArray(topic.subcontacts)?topic.subcontacts:[];
  items.forEach(item=>{
    const exists=topic.subcontacts.some(saved=>saved.name.toLocaleUpperCase('pt-BR')===item.name.toLocaleUpperCase('pt-BR')&&cleanPhone(saved.phone)===cleanPhone(item.phone));
    if(!exists)topic.subcontacts.push({...item,id:Math.max(0,...topic.subcontacts.map(saved=>Number(saved.id)||0))+1});
  });
}
...
function displayCategory(category){ return categoryLabels[category]||category; }
...
function filteredContacts(){
  ...
  return sorted(contacts.filter(c=>!q||[c.name,c.role,c.phone,c.category,displayCategory(c.category),...(c.subcontacts||[]).flatMap(item=>[item.name,item.role,item.phone,item.ramal,item.sso])].some(v=>String(v||'').toLocaleLowerCase('pt-BR').includes(q))));
}
function contactCard(contact){
  ...
  const info=el('span','contact-info'); info.append(el('strong','',contact.name),el('span','',contact.role||contact.category)); if(contact.phone&&contact.category!=='Operacional')info.append(el('small','',contact.phone));
  ...
  topicTools.append(addContact,editTopic); contentHeader.append(headerCopy,topicTools); content.append(contentHeader);
  ...
  content.append(subcontactList); card.append(icon,info,indicator); item.append(card,content);
...
function subcontactRow(parent,subcontact){
  ...
  const copy=el('div','subcontact-copy'); copy.append(el('strong','',subcontact.name),el('span','',subcontact.role||'Contato'));
  if(subcontact.phone) copy.append(el('small','',`TEL: ${subcontact.phone}`));
  if(subcontact.ramal) copy.append(el('small','',`RAMAL SE: ${subcontact.ramal}`));
  if(subcontact.sso) copy.append(el('small','',`SSO: ${subcontact.sso}`));
  ...
  const edit=el('button','subcontact-action edit-action','Editar'); edit.type='button'; edit.addEventListener('click',()=>requireAdministrator(()=>openSubcontactForm(parent,subcontact)));
  const remove=el('button','subcontact-action delete-action','Excluir'); remove.type='button'; remove.addEventListener('click',()=>requireAdministrator(()=>deleteSubcontact(parent,subcontact)));
  actions.append(call,whatsapp,edit,remove); row.append(copy,actions); return row;
}
function renderDirectory(){
  const list=filteredContacts(), root=$('#contact-sections'), searching=Boolean($('#search').value.trim()); root.replaceChildren();
  $('#contact-count').textContent=contacts.length; $('#empty-state').classList.toggle('hidden',list.length>0);
  categoryOrder.concat([...new Set(list.map(c=>c.category).filter(c=>!categoryOrder.includes(c)))].sort()).forEach(category=>{
    ...
    const title=el('span','category-title',displayCategory(category)); const count=el('span','category-count',`${items.length} contato${items.length===1?'':'s'}`); const chevron=el('span','category-chevron','⌄');
...
function renderAdmin(){
  ...
  sorted(contacts).forEach(contact=>{
    ...
    info.append(el('strong','',contact.name),el('span','',[contact.role||'Sem descrição',contact.phone,displayCategory(contact.category),`${contact.subcontacts.length} internos`].filter(Boolean).join(' · ')));
...
function deleteContact(contact){ if(confirm(`Deseja remover “${contact.name}”?`)){ contacts=contacts.filter(c=>c.id!==contact.id); saveContacts(); renderDirectory(); renderAdmin(); } }
function openContactForm(contact){
  ...
  $('#contact-category').value=contact?.category||'Emergência'; $('#contact-icon').value=contact?.icon||'phone'; $('#contact-order').value=contact?.order??0; openModal('form-modal'); setTimeout(()=>$('#contact-name').focus(),50);
}
function openSubcontactForm(parent,subcontact){
  ...
  $('#subcontact-name').value=subcontact?.name||''; $('#subcontact-role').value=subcontact?.role||''; $('#subcontact-phone').value=subcontact?.phone||''; $('#subcontact-ramal').value=subcontact?.ramal||''; $('#subcontact-sso').value=subcontact?.sso||'';
...
function deleteSubcontact(parent,subcontact){
  ...
  contacts=contacts.map(contact=>sameId(contact.id,parent.id)?{...contact,subcontacts:contact.subcontacts.filter(item=>!sameId(item.id,subcontact.id))}:contact);
  saveContacts(); renderDirectory(); renderAdmin();
}
...
$('#contact-form').addEventListener('submit',event=>{
  event.preventDefault(); const id=$('#contact-id').value; const previous=contacts.find(contact=>sameId(contact.id,id)); const category=$('#contact-category').value; const data={id:id||Math.max(0,...contacts.map(c=>Number(c.id)||0))+1,name:$('#contact-name').value.trim(),role:$('#contact-role').value.trim(),phone:category==='Operacional'?'':$('#contact-phone').value.trim(),category,icon:$('#contact-icon').value,order:Number($('#contact-order').value)||0,subcontacts:previous?.subcontacts||[]};
  contacts=id?contacts.map(c=>sameId(c.id,id)?data:c):contacts.concat(data); saveContacts(); closeModal('form-modal'); renderAdmin(); renderDirectory();
});
$('#subcontact-form').addEventListener('submit',event=>{
  event.preventDefault(); const parentId=$('#subcontact-parent-id').value; const subcontactId=$('#subcontact-id').value;
  const data={id:subcontactId||0,name:$('#subcontact-name').value.trim(),role:$('#subcontact-role').value.trim(),phone:$('#subcontact-phone').value.trim(),ramal:$('#subcontact-ramal').value.trim(),sso:$('#subcontact-sso').value.trim()};
  contacts=contacts.map(contact=>{
    if(!sameId(contact.id,parentId))return contact;
    const nextId=subcontactId||Math.max(0,...contact.subcontacts.map(item=>Number(item.id)||0))+1; const saved={...data,id:nextId};
    return {...contact,subcontacts:subcontactId?contact.subcontacts.map(item=>sameId(item.id,subcontactId)?saved:item):contact.subcontacts.concat(saved)};
  });
  openContacts.add(parentId); saveContacts(); closeModal('subcontact-form-modal'); renderAdmin(); renderDirectory();
});
document.addEventListener('click',event=>{ const id=event.target.dataset.close; if(id){ if(id==='login-modal')pendingAdministratorAction=null; closeModal(id); } });
